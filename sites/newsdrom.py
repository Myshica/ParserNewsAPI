...
class NewsDrom(BaseParser):

    DEFAULT_URL = "https://news.drom.ru"
    ...
    async def start(self, soup: BeautifulSoup, cat: str) -> News | None:
        try:
            news_title = soup.find('div', class_='b-info-block__title b-link').text.strip()
            news_url = soup.find('a', class_='b-info-block__cont b-info-block__cont_state_reviews').get('href')
            news_date = soup.find('div', class_='b-info-block__text b-info-block__text_type_news-date').text.strip()
    ...
    async def open(self, other_url: str) -> tuple[str, str | list[str] | None] | tuple[None, None]:
        ...
        try:
            ...
            photo = soup.find('img', class_="b-image b-image_responsive").get("src")
            return desc, photo
        except Exception as e:
